        ...
        <div class="col-lg-9 col-xl-8">
            ...
            <div class="d-flex align-items-center justify-content-between mb-4 border-bottom pb-3">
                <h2 class="history-title mb-0" style="font-size: 1.5rem; font-weight: 700; color: #1e3c72;">
                    <i class="fas fa-plus-circle text-primary me-2"></i> Tambah Informasi Cafe
                </h2>
            ...
            <div class="card shadow-sm border-0 p-4" style="border-radius: 20px; background: #ffffff;">
                <form enctype="multipart/form-data" method="POST" action="{{ route('tentangs.store') }}" class="d-flex flex-column gap-4">
                    @csrf

                    <div>
                        <label for="judul" class="form-label fw-bold text-dark mb-2" style="font-size: 0.85rem;">Judul Artikel / Informasi</label>
                        <input id="judul" class="form-control" type="text" name="judul" placeholder="Masukkan judul deskripsi tentang..." value="{{ old('judul') }}" required />
                    </div>

                    <div>
                        <label for="deskripsi" class="form-label fw-bold text-dark mb-2" style="font-size: 0.85rem;">Deskripsi / Cerita Profil</label>
                        <textarea id="deskripsi" class="form-control" name="deskripsi" placeholder="Tuliskan cerita lengkap tentang profil DelCafe..." rows="6" required>{{ old('deskripsi') }}</textarea>
                    </div>
                    ...
                    <div class="pt-3 border-top mt-2">
                        <button type="submit" class="btn btn-primary w-100 py-2.5 rounded-pill" style="font-size: 0.9rem; font-weight: 600;">
                            <i class="fas fa-save me-1.5"></i> Simpan Informasi Baru
                        </button>
                    </div>
                </form>
            </div>
